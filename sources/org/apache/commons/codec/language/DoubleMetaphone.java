...
public class DoubleMetaphone implements StringEncoder {
    ...
    private static final String[] L_R_N_M_B_H_F_V_W_SPACE = {"L", "R", "N", "M", "B", "H", "F", GpsStatus.INTEROPERABILITY, GpsLongitudeRef.WEST, MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR};
    private static final String[] L_T_K_S_N_M_B_Z = {"L", GpsTrackRef.TRUE_DIRECTION, GpsSpeedRef.KILOMETERS, GpsLatitudeRef.SOUTH, "N", "M", "B", "Z"};
            ...
            if (contains(value, index, 2, "CH")) {
                ...
                if (contains(value, index + 1, 3, "CIA")) {
                    ...
                    if (contains(value, index, 2, "CK", "CG", "CQ")) {
                        ...
                        if (contains(value, index, 2, "CI", BleEnum.BLE_SCALE_TYPE_CE, "CY")) {
                            ...
                            if (contains(value, index + 1, 2, " C", " Q", " G")) {
                                ...
                                if (contains(value, index + 1, 1, "C", GpsSpeedRef.KILOMETERS, "Q")) {
                                    if (!contains(value, index + 1, 2, BleEnum.BLE_SCALE_TYPE_CE, "CI")) {
                                        index2 = index + 2;
                                    }
                                }
                                index2 = index + 1;
    ...
    private int handleCC(String value, DoubleMetaphoneResult result, int index) {
        if (contains(value, index + 2, 1, "I", GpsLongitudeRef.EAST, "H")) {
    ...
    private int handleD(String value, DoubleMetaphoneResult result, int index) {
        if (contains(value, index, 2, "DG")) {
            if (contains(value, index + 2, 1, "I", GpsLongitudeRef.EAST, "Y")) {
                result.append('J');
                return index + 3;
            }
            result.append("TK");
        ...
        if (contains(value, index + 1, 2, "LI") && !slavoGermanic) {
            ...
            if (contains(value, index + 1, 2, "ER") || charAt(value, index + 1) == 'Y') {
                if (!contains(value, 0, 6, "DANGER", "RANGER", "MANGER")) {
                    if (!contains(value, index - 1, 1, GpsLongitudeRef.EAST, "I")) {
            ...
            if (!contains(value, index + 1, 1, GpsLongitudeRef.EAST, "I", "Y")) {
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:24:0x00bb, code lost:
        if (contains(r10, r12 + 1, 1, "M", "N", "L", com.iwown.device_module.device_camera.exif.ExifInterface.GpsLongitudeRef.WEST) == false) goto L_0x00bd;
     */
    ...
    private int handleT(String value, DoubleMetaphoneResult result, int index) {
        ...
        if (!contains(value, index, 2, "TH")) {
            if (!contains(value, index, 3, "TTH")) {
                result.append('T');
                return contains(value, index + 1, 1, GpsTrackRef.TRUE_DIRECTION, "D") ? index + 2 : index + 1;
            }
        }
    ...
    private int handleZ(String value, DoubleMetaphoneResult result, int index, boolean slavoGermanic) {
        ...
        if (contains(value, index + 1, 2, "ZO", "ZI", "ZA") || (slavoGermanic && index > 0 && charAt(value, index - 1) != 'T')) {
            result.append(GpsLatitudeRef.SOUTH, "TS");
        } else {
            result.append('S');
        }
        ...
        if (!contains(value, 0, 4, "VAN ", "VON ")) {
            if (!contains(value, 0, 3, "SCH")) {
                if (!contains(value, index - 2, 6, "ORCHES", "ARCHIT", "ORCHID")) {
                    if (!contains(value, index + 2, 1, GpsTrackRef.TRUE_DIRECTION, GpsLatitudeRef.SOUTH)) {
                        if (!contains(value, index - 1, 1, GpsStatus.IN_PROGRESS, "O", "U", GpsLongitudeRef.EAST) && index != 0) {
                            return false;
                        }
    ...
    /* JADX WARNING: Code restructure failed: missing block: B:7:0x0055, code lost:
        if (contains(r8, r8.length() - 1, 1, com.iwown.device_module.device_camera.exif.ExifInterface.GpsStatus.IN_PROGRESS, "O") != false) goto L_0x0057;
     */
    /* Code decompiled incorrectly, please refer to instructions dump. */

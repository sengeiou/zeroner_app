...
public class Sha2Crypt {
    ...
    private static String sha2Crypt(byte[] keyBytes, String salt, String saltPrefix, int blocksize, String algorithm) {
        ...
        for (int i2 = 1; i2 <= (altResult2[0] & UnsignedBytes.MAX_VALUE) + 16; i2++) {
        ...
        if (blocksize == 32) {
            ...
            B64.b64from24bit(0, altResult2[31], altResult2[30], 3, buffer);
        } else {
            ...
            B64.b64from24bit(0, 0, altResult2[63], 2, buffer);
        }
        Arrays.fill(tempResult2, 0);
        Arrays.fill(pBytes, 0);
        Arrays.fill(sBytes, 0);
        ctx.reset();
        altCtx3.reset();
        Arrays.fill(keyBytes, 0);
        Arrays.fill(saltBytes, 0);

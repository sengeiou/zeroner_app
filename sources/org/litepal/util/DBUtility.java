...
public class DBUtility {
    ...
    public static String getGenericTableName(String className, String fieldName) {
        return BaseUtility.changeCase(getTableNameByClassName(className) + "_" + fieldName);
    }
    ...
    public static boolean isIntermediateTable(String tableName, SQLiteDatabase db) {
        if (!TextUtils.isEmpty(tableName) && tableName.matches("[0-9a-zA-Z]+_[0-9a-zA-Z]+")) {
            Cursor cursor = null;
            try {
                cursor = db.query(TableSchema.TABLE_NAME, null, null, null, null, null, null);
                if (cursor.moveToFirst()) {
                    while (true) {
                        if (!tableName.equalsIgnoreCase(cursor.getString(cursor.getColumnIndexOrThrow(TableSchema.COLUMN_NAME)))) {
    ...
    public static boolean isColumnExists(String columnName, String tableName, SQLiteDatabase db) {
        ...
        try {
            ...
            if (cursor.moveToFirst()) {
                while (true) {
                    if (!columnName.equalsIgnoreCase(cursor.getString(cursor.getColumnIndexOrThrow(TableSchema.COLUMN_NAME)))) {
                        if (!cursor.moveToNext()) {
                            break;
                        }
                    } else {
    ...
    public static TableModel findPragmaTableInfo(String tableName, SQLiteDatabase db) {
        ...
        if (isTableExists(tableName, db)) {
            ...
            Cursor cursor = null;
            try {
                Cursor cursor2 = db.rawQuery("pragma table_info(" + tableName + ")", null);
                if (cursor2.moveToFirst()) {
                    do {
                        ColumnModel columnModel = new ColumnModel();
                        String name = cursor2.getString(cursor2.getColumnIndexOrThrow(TableSchema.COLUMN_NAME));
    ...
    public static List<String> findUniqueColumns(String tableName, SQLiteDatabase db) {
        ...
        try {
            Cursor cursor2 = db.rawQuery("pragma index_list(" + tableName + ")", null);
            if (cursor2.moveToFirst()) {
                do {
                    if (cursor2.getInt(cursor2.getColumnIndexOrThrow("unique")) == 1) {
                        innerCursor = db.rawQuery("pragma index_info(" + cursor2.getString(cursor2.getColumnIndexOrThrow(TableSchema.COLUMN_NAME)) + ")", null);
                        if (innerCursor.moveToFirst()) {
                            columns.add(innerCursor.getString(innerCursor.getColumnIndexOrThrow(TableSchema.COLUMN_NAME)));
                        }
                    }
                } while (cursor2.moveToNext());
            }
    ...
    public static boolean isFieldNameConflictWithSQLiteKeywords(String fieldName) {
        if (!TextUtils.isEmpty(fieldName)) {
            if (SQLITE_KEYWORDS.contains("," + fieldName.toLowerCase() + ",")) {
                return true;
            }
        }
        return false;
    ...
    public static String convertWhereClauseToColumnName(String whereClause) {
        try {
            ...
            while (m.find()) {
                ...
                m.appendReplacement(convertedWhereClause, convertToValidColumnName(column) + matches.replace(column, ""));
            }
    ...
    private static String convertOrderByItem(String orderByItem) {
        ...
        if (orderByItem.endsWith("asc")) {
            ...
            append = " asc";
        } else if (orderByItem.endsWith(SocialConstants.PARAM_APP_DESC)) {
            column = orderByItem.replace(SocialConstants.PARAM_APP_DESC, "").trim();
            append = " desc";
        } else {
            column = orderByItem;
            append = "";

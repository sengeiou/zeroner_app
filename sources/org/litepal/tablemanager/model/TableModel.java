...
public class TableModel {
        ...
        if (!TextUtils.isEmpty(columnName)) {
            ...
            while (true) {
                if (i >= this.columnModels.size()) {
                    break;
                } else if (columnName.equalsIgnoreCase(((ColumnModel) this.columnModels.get(i)).getColumnName())) {
                    indexToRemove = i;
                    break;
                } else {
                    i++;
    ...
    public boolean containsColumn(String columnName) {
        for (int i = 0; i < this.columnModels.size(); i++) {
            if (columnName.equalsIgnoreCase(((ColumnModel) this.columnModels.get(i)).getColumnName())) {
                return true;
            }
        }
        return false;

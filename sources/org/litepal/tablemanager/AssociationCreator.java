...
public abstract class AssociationCreator extends Generator {
    ...
    public String generateCreateTableSQL(String tableName, List<ColumnModel> columnModels, boolean autoIncrementId) {
        ...
        for (ColumnModel columnModel : columnModels) {
            if (!columnModel.isIdColumn()) {
                ...
                needSeparator = true;
                createTableSQL.append(columnModel.getColumnName()).append(MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR).append(columnModel.getColumnType());
    ...
    public String generateAddColumnSQL(String tableName, ColumnModel columnModel) {
        ...
        addColumnSQL.append(MinimalPrettyPrinter.DEFAULT_ROOT_VALUE_SEPARATOR).append(columnModel.getColumnType());
        if (!columnModel.isNullable()) {
    ...
    public void giveTableSchemaACopy(String tableName, int tableType, SQLiteDatabase db) {
        ...
        sql.append(TableSchema.TABLE_NAME);
        LogUtil.d(Generator.TAG, "giveTableSchemaACopy SQL is >> " + sql);
        Cursor cursor = null;
        try {
            Cursor cursor2 = db.rawQuery(sql.toString(), null);
            if (isNeedtoGiveACopy(cursor2, tableName)) {
                ContentValues values = new ContentValues();
                values.put(TableSchema.COLUMN_NAME, BaseUtility.changeCase(tableName));
                values.put("type", Integer.valueOf(tableType));
                db.insert(TableSchema.TABLE_NAME, null, values);
            }
            if (cursor2 != null) {
                cursor2.close();
            }
    ...
    private boolean isValueExists(Cursor cursor, String tableName) {
        ...
        while (!cursor.getString(cursor.getColumnIndexOrThrow(TableSchema.COLUMN_NAME)).equalsIgnoreCase(tableName)) {
    ...
    private boolean isSpecialTable(String tableName) {
        return TableSchema.TABLE_NAME.equalsIgnoreCase(tableName);
    }
    ...
    private boolean isContainsOnlyIdField(List<ColumnModel> columnModels) {
        if (columnModels.size() == 0 || ((columnModels.size() == 1 && isIdColumn(((ColumnModel) columnModels.get(0)).getColumnName())) || (columnModels.size() == 2 && isIdColumn(((ColumnModel) columnModels.get(0)).getColumnName()) && isIdColumn(((ColumnModel) columnModels.get(1)).getColumnName())))) {
            return true;
        }
        return false;
    }

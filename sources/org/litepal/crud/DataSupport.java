...
public class DataSupport {
    ...
    public static <T> FindExecutor maxAsync(final String tableName, final String columnName, final Class<T> columnType) {
        ...
        executor.submit(new Runnable() {
            public void run() {
                synchronized (DataSupport.class) {
                    final T t = DataSupport.max(tableName, columnName, columnType);
                    if (executor.getListener() != null) {
                        LitePal.getHandler().post(new Runnable() {
                            public void run() {
    ...
    public static <T> FindExecutor minAsync(final String tableName, final String columnName, final Class<T> columnType) {
        ...
        executor.submit(new Runnable() {
            public void run() {
                synchronized (DataSupport.class) {
                    final T t = DataSupport.min(tableName, columnName, columnType);
                    if (executor.getListener() != null) {
                        LitePal.getHandler().post(new Runnable() {
                            public void run() {
                                executor.getListener().onFinish(t);
    ...
    public static <T> FindExecutor sumAsync(final String tableName, final String columnName, final Class<T> columnType) {
        ...
        executor.submit(new Runnable() {
            public void run() {
                synchronized (DataSupport.class) {
                    final T t = DataSupport.sum(tableName, columnName, columnType);
                    if (executor.getListener() != null) {
                        LitePal.getHandler().post(new Runnable() {
                            public void run() {
                                executor.getListener().onFinish(t);
    ...
    public static <T> FindExecutor findAsync(Class<T> modelClass, long id, boolean isEager) {
        final FindExecutor executor = new FindExecutor();
        final Class<T> cls = modelClass;
        final long j = id;
        final boolean z = isEager;
        executor.submit(new Runnable() {
            public void run() {
                synchronized (DataSupport.class) {
                    final T t = DataSupport.find(cls, j, z);
                    if (executor.getListener() != null) {
                        LitePal.getHandler().post(new Runnable() {
                            public void run() {
                                executor.getListener().onFinish(t);
    ...
    public static <T> FindExecutor findFirstAsync(final Class<T> modelClass, final boolean isEager) {
        ...
        executor.submit(new Runnable() {
            public void run() {
                synchronized (DataSupport.class) {
                    final T t = DataSupport.findFirst(modelClass, isEager);
                    if (executor.getListener() != null) {
                        LitePal.getHandler().post(new Runnable() {
                            public void run() {
                                executor.getListener().onFinish(t);
    ...
    public static <T> FindExecutor findLastAsync(final Class<T> modelClass, final boolean isEager) {
        ...
        executor.submit(new Runnable() {
            public void run() {
                synchronized (DataSupport.class) {
                    final T t = DataSupport.findLast(modelClass, isEager);
                    if (executor.getListener() != null) {
                        LitePal.getHandler().post(new Runnable() {
                            public void run() {
                                executor.getListener().onFinish(t);
    ...
    public static <T> FindMultiExecutor findAllAsync(final Class<T> modelClass, final boolean isEager, final long... ids) {
        ...
        executor.submit(new Runnable() {
            public void run() {
                synchronized (DataSupport.class) {
                    final List<T> t = DataSupport.findAll(modelClass, isEager, ids);
                    if (executor.getListener() != null) {
                        LitePal.getHandler().post(new Runnable() {
                            public void run() {
                                executor.getListener().onFinish(t);
    ...
    public static UpdateOrDeleteExecutor updateAsync(Class<?> modelClass, ContentValues values, long id) {
        final UpdateOrDeleteExecutor executor = new UpdateOrDeleteExecutor();
        ...
        executor.submit(new Runnable() {
            public void run() {
                synchronized (DataSupport.class) {
                    final int rowsAffected = DataSupport.update(cls, contentValues, j);
                    if (executor.getListener() != null) {
                        LitePal.getHandler().post(new Runnable() {
                            public void run() {
                                executor.getListener().onFinish(rowsAffected);
    ...
    public static <T> boolean isExist(Class<T> modelClass, String... conditions) {
        if (conditions != null && where(conditions).count(modelClass) > 0) {
            return true;
        }
        return false;
    }

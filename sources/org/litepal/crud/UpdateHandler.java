...
class UpdateHandler extends DataHandler {
    ...
    private void updateSelfTableForeignKey(DataSupport baseObj, ContentValues values) {
        ...
        for (String associatedTable : associatedModelMap.keySet()) {
            values.put(getForeignKeyColumnName(associatedTable), (Long) associatedModelMap.get(associatedTable));
        }
    }
    ...
    private int updateAssociatedTableForeignKey(DataSupport baseObj, long id) {
        ...
        for (String associatedTable : associatedModelMap.keySet()) {
            ...
            Set<Long> ids = (Set) associatedModelMap.get(associatedTable);
            if (ids != null && !ids.isEmpty()) {
                return this.mDatabase.update(associatedTable, values, getWhereOfIdsWithOr((Collection<Long>) ids), null);
            }
        }
        return 0;
    }

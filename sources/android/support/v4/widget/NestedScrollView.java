...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent, NestedScrollingChild2, ScrollingView {
        ...
        if (canScroll()) {
            ...
            if (event.getAction() == 0) {
                switch (event.getKeyCode()) {
                    ...
                    case 20:
                        if (event.isAltPressed()) {
                            handled = fullScroll(Opcodes.INT_TO_FLOAT);
                            break;
                        } else {
                            handled = arrowScroll(Opcodes.INT_TO_FLOAT);
                            break;
                        }
                    case 62:
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, Opcodes.INT_TO_FLOAT);
            if (nextFocused == null || nextFocused == this || !nextFocused.requestFocus(Opcodes.INT_TO_FLOAT)) {
                return false;
            }
            return true;
        }
    ...
    public boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = Opcodes.INT_TO_FLOAT;
        } else if (direction == 1) {
            direction = 33;
        }

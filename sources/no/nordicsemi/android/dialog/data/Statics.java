...
public class Statics {
    ...
    public static int gpioStringToInt(String spioValue) {
        return Integer.decode("0x" + spioValue.replace("P", "").replace("_", "")).intValue();
    }

    public static String getPreviousInput(Context c, int resourceID) {
        return c.getSharedPreferences("settings", 0).getString(String.valueOf(resourceID), "0");
    }

    public static Map<String, ?> getAllPreviousInput(Context c) {
        return c.getSharedPreferences("settings", 0).getAll();
    }

    public static void setPreviousInput(Context c, int resourceID, String value) {
        c.getSharedPreferences("settings", 0).edit().putString(String.valueOf(resourceID), value.trim()).commit();
    }

    public static boolean fileDirectoriesCreated(Context c) {
        return c.getSharedPreferences("settings", 0).getBoolean("fileDirectoriesCreated", false);
    }

    public static void setFileDirectoriesCreated(Context c) {
        c.getSharedPreferences("settings", 0).edit().putBoolean("fileDirectoriesCreated", true).commit();
    }
}

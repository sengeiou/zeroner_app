...
public class PositionUtil {
    ...
    public static boolean outOfChina(double lat, double lon) {
        ...
        if (TextUtils.isEmpty(country)) {
            if (Rectangle.IsInsideChina(new Gps(lat, lon))) {
                return false;
            }
            return true;
    ...
    public static Gps transformMust(double lat, double lon) {
        double dLat = transformLat(lon - 105.0d, lat - 35.0d);
        double dLon = transformLon(lon - 105.0d, lat - 35.0d);
        double radLat = (lat / 180.0d) * pi;
        double magic = Math.sin(radLat);
        double magic2 = 1.0d - ((ee * magic) * magic);
        double sqrtMagic = Math.sqrt(magic2);
        return new Gps(lat + ((180.0d * dLat) / (((a * (1.0d - ee)) / (magic2 * sqrtMagic)) * pi)), lon + ((180.0d * dLon) / (((a / sqrtMagic) * Math.cos(radLat)) * pi)));
    }
    ...
    public static Gps transform2(double lat, double lon) {
        if (outOfChina(lat, lon)) {
            Gps gps = new Gps(lat, lon);
            return gps;
        }
    ...
    public static double[] initLocation(Context context) {
        ...
        if (locationManager.isProviderEnabled("gps")) {
            ...
            locationManager.requestLocationUpdates("network", 1000, 0.0f, new LocationListener() {
            ...
            if (location2 != null) {
                ...
                gpsLocation[0] = location2.getLongitude();
                gpsLocation[1] = latitude2;
            }
        }
        return gpsLocation;
    }

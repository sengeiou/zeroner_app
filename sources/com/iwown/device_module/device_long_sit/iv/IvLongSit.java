package com.iwown.device_module.device_long_sit.iv;
...
public class IvLongSit {
    ...
    public static IvLongSit instance;

    private IvLongSit() {
    }

    public static IvLongSit getInstance() {
        if (instance == null) {
            instance = new IvLongSit();
        }
        return instance;
    }
    ...
    public void writeDataToPedomater(Context context, int startHour, int endHour, int index, int repeat) {
        ...
        data[6] = (byte) high;
        sendentary.add(data);
        byte[] data1 = ZeronerSendBluetoothCmdImpl.getInstance().setSedentary(data[0], data[1], data[2], data[3], data[4], 50);
        DataBean dataBean = new DataBean();
        dataBean.addData(data1);
        ZeronerBackgroundThreadManager.getInstance().addWriteData(context, dataBean);
    }

    private void writeSendentary(Context context, int startHour, int endHour, int repeat) {
        byte[] data = {0, 0, 0, 0, 0, 0, 0};
        data[0] = 0;
        data[1] = (byte) repeat;

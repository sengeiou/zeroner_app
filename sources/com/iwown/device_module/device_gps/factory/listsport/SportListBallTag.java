...
public class SportListBallTag extends SportListAllExecutor {
    public List<CopySportGps> getCopySportGpsList(long uid, long startTime, int count, int sportType) {
        ...
        List<TB_sport_ball> gps_segmentList = DataSupport.where("uid=? and start_time<?", uid + "", startTime + "").order("start_time desc").find(TB_sport_ball.class);
        if (gps_segmentList != null && gps_segmentList.size() > 0) {
            List<TB_sport_ball> gps_segments = new LinkedList<>();
            if (gps_segments.size() <= count) {
                gps_segments.addAll(gps_segmentList);
            } else {
                gps_segments.addAll(gps_segmentList.subList(0, count));
            }
            for (TB_sport_ball gps_segment : gps_segments) {

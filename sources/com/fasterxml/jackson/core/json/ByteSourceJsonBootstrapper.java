...
public final class ByteSourceJsonBootstrapper {
    ...
    public JsonEncoding detectEncoding() throws IOException, JsonParseException {
        ...
        if (ensureLoaded(4)) {
            int quad = (this._inputBuffer[this._inputPtr] << Ascii.CAN) | ((this._inputBuffer[this._inputPtr + 1] & UnsignedBytes.MAX_VALUE) << 16) | ((this._inputBuffer[this._inputPtr + 2] & UnsignedBytes.MAX_VALUE) << 8) | (this._inputBuffer[this._inputPtr + 3] & 255);
            if (handleBOM(quad)) {
                foundEncoding = true;
        ...
        } else if (ensureLoaded(2) && checkUTF16(((this._inputBuffer[this._inputPtr] & UnsignedBytes.MAX_VALUE) << 8) | (this._inputBuffer[this._inputPtr + 1] & 255))) {
            foundEncoding = true;
        }
        if (!foundEncoding) {
    ...
    private static int skipSpace(InputAccessor acc, byte b) throws IOException {
        while (true) {
            byte ch = b & UnsignedBytes.MAX_VALUE;

...
final class MessageLiteToString {
    ...
    private static void reflectivePrintWithIndent(MessageLite messageLite, StringBuilder buffer, int indent) {
        ...
        if (messageLite instanceof ExtendableMessage) {
            Iterator<Entry<ExtensionDescriptor, Object>> iter = ((ExtendableMessage) messageLite).extensions.iterator();
            while (iter.hasNext()) {
                Entry<ExtensionDescriptor, Object> entry = (Entry) iter.next();
                printField(buffer, indent, "[" + ((ExtensionDescriptor) entry.getKey()).getNumber() + "]", entry.getValue());
            }
        }
    ...
    private static boolean isDefaultValue(Object o) {
        ...
        if (o instanceof Boolean) {
            ...
            if (((Double) o).doubleValue() != Utils.DOUBLE_EPSILON) {
                return false;
            }
            return true;
    ...
    static final void printField(StringBuilder buffer, int indent, String name, Object object) {
        if (object instanceof List) {
            ...
            for (Entry<?, ?> entry2 : ((Map) object).entrySet()) {

...
public class TinkerZipOutputStream extends FilterOutputStream implements ZipConstants {
    ...
    public TinkerZipOutputStream(OutputStream os, boolean forceZip642) {
        super(os);
    ...
    public void closeEntry() throws IOException {
        ...
        if (this.currentEntry != null) {
            ...
            if (this.currentEntry.getMethod() == 8) {
                curOffset = curOffset2 + this.currentEntry.getCompressedSize();
            } else {
                curOffset = curOffset2 + this.currentEntry.getSize();
            }
            writeLongAsUint32(this.cDir, this.currentEntry.getCompressedSize());
            writeLongAsUint32(this.cDir, this.currentEntry.getSize());
            long curOffset3 = curOffset + ((long) writeIntAsUint16(this.cDir, this.nameBytes.length));
            if (this.currentEntry.extra != null) {
                curOffset3 += (long) writeIntAsUint16(this.cDir, this.currentEntry.extra.length);
            } else {
    ...
    public void write(byte[] buffer, int offset2, int byteCount) throws IOException {
        Arrays.checkOffsetAndCount(buffer.length, offset2, byteCount);
        if (this.currentEntry == null) {
            throw new ZipException("No active entry");
        } else if (this.currentEntry.getMethod() == 0) {
            this.out.write(buffer, offset2, byteCount);
        } else {
            this.out.write(buffer, offset2, byteCount);
        }
    }

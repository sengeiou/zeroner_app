...
public class DexDataBuffer implements ByteInput, ByteOutput {
    ...
    public DexDataBuffer(ByteBuffer data2) {
        this.data = data2;
        this.data.order(ByteOrder.LITTLE_ENDIAN);
        this.dataBound = data2.limit();
        this.isResizeAllowed = false;
    }

    public DexDataBuffer(ByteBuffer data2, boolean isResizeAllowed2) {
        this.data = data2;
        this.data.order(ByteOrder.LITTLE_ENDIAN);
        this.dataBound = data2.limit();
        this.isResizeAllowed = isResizeAllowed2;
    }
        ...
        try {
            ...
            try {
                ...
                while (true) {
                    ...
                    switch (opcode) {
                        case 0:
                            DebugInfoItem debugInfoItem = new DebugInfoItem(off, lineStart, parameterNames, baos2.toByteArray());
                            if (baos2 != null) {
                                try {
                                    baos2.close();
                                } catch (Exception e) {
    ...
    public AnnotationsDirectory readAnnotationsDirectory() {
        ...
        int[][] fieldAnnotations = (int[][]) Array.newInstance(Integer.TYPE, new int[]{fieldsSize, 2});
        for (int i = 0; i < fieldsSize; i++) {
            fieldAnnotations[i][0] = readInt();
            fieldAnnotations[i][1] = readInt();
        }
        int[][] methodAnnotations = (int[][]) Array.newInstance(Integer.TYPE, new int[]{methodsSize, 2});
        for (int i2 = 0; i2 < methodsSize; i2++) {
            methodAnnotations[i2][0] = readInt();
            methodAnnotations[i2][1] = readInt();
        }
        int[][] parameterAnnotations = (int[][]) Array.newInstance(Integer.TYPE, new int[]{parameterListSize, 2});
        for (int i3 = 0; i3 < parameterListSize; i3++) {
            parameterAnnotations[i3][0] = readInt();
            parameterAnnotations[i3][1] = readInt();
        }

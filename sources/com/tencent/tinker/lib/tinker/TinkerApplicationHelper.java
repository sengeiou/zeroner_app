...
public class TinkerApplicationHelper {
    ...
    public static String getCurrentVersion(ApplicationLike applicationLike) {
        ...
        String oldVersion = ShareIntentUtil.getStringExtra(tinkerResultIntent, ShareIntentUtil.INTENT_PATCH_OLD_VERSION);
        String newVersion = ShareIntentUtil.getStringExtra(tinkerResultIntent, ShareIntentUtil.INTENT_PATCH_NEW_VERSION);
        boolean isMainProcess = ShareTinkerInternals.isInMainProcess(applicationLike.getApplication());
    ...
    public static void loadArmV7aLibrary(ApplicationLike applicationLike, String libName) {
        ...
        } else if (!isTinkerEnableForNativeLib(applicationLike) || !loadLibraryFromTinker(applicationLike, "lib/armeabi-v7a", libName)) {
    ...
    public static void loadArmLibrary(ApplicationLike applicationLike, String libName) {
        if (libName == null || libName.isEmpty() || applicationLike == null) {
            throw new TinkerRuntimeException("libName or context is null!");
        } else if (!isTinkerEnableForNativeLib(applicationLike) || !loadLibraryFromTinker(applicationLike, "lib/armeabi", libName)) {
            System.loadLibrary(libName);
        }
    }
    ...
    public static boolean loadLibraryFromTinker(ApplicationLike applicationLike, String relativePath, String libname) throws UnsatisfiedLinkError {
        ...
        String libPrePath = new File(patchDirectory.getAbsolutePath() + "/" + SharePatchFileUtil.getPatchVersionDirectory(currentVersion)).getAbsolutePath() + "/" + ShareConstants.SO_PATH;
        for (Entry<String, String> libEntry : loadLibraries.entrySet()) {
            String name = (String) libEntry.getKey();
            if (name.equals(relativeLibPath)) {
                String patchLibraryPath = libPrePath + "/" + name;
                File library = new File(patchLibraryPath);
                if (!library.exists()) {
                    continue;
                } else if (!applicationLike.getTinkerLoadVerifyFlag() || SharePatchFileUtil.verifyFileMd5(library, (String) loadLibraries.get(name))) {

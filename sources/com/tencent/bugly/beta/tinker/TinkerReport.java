...
public class TinkerReport {
    ...
    public static void onLoaded(long cost) {
        if (reporter != null) {
            ...
            } else if (cost <= BootloaderScanner.TIMEOUT) {
            ...
            switch (errorCode) {
                ...
                case -3:
                    if (!throwable.getMessage().contains(ShareConstants.CHECK_RES_INSTALL_FAIL)) {
                        ...
                        TinkerLog.e(TAG, "tinker res reflect fail:" + throwable.getMessage(), new Object[0]);
                        z = false;
                        break;
                    } else {
                        reporter.onReport(255);
                        TinkerLog.e(TAG, "tinker res check fail:" + throwable.getMessage(), new Object[0]);
                        break;
                    }
                case -2:
                    if (!throwable.getMessage().contains(ShareConstants.CHECK_DEX_INSTALL_FAIL)) {
                        reporter.onReport((int) KEY_LOADED_EXCEPTION_DEX);
                        TinkerLog.e(TAG, "tinker dex reflect fail:" + throwable.getMessage(), new Object[0]);
                        z = false;
                        break;
                    } else {
                        reporter.onReport((int) KEY_LOADED_EXCEPTION_DEX_CHECK);
                        TinkerLog.e(TAG, "tinker dex check fail:" + throwable.getMessage(), new Object[0]);
                        break;
                    }
                case -1:

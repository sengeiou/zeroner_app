...
public class AnimatedGifEncoder {
    ...
    public void setSize(int w, int h) {
        if (!this.started || this.firstFrame) {
            ...
            this.height = h;
            if (this.width < 1) {
                this.width = WearableManager.VERSION_38;
            }
            if (this.height < 1) {
                this.height = 240;
            }
    ...
    private void analyzePixels() {
        ...
        while (i2 < nPix) {
            ...
            int k3 = k2 + 1;
            int k4 = k3 + 1;
            int index = nq.map(this.pixels[k] & UnsignedBytes.MAX_VALUE, this.pixels[k2] & UnsignedBytes.MAX_VALUE, this.pixels[k3] & UnsignedBytes.MAX_VALUE);
    ...
    private int findClosest(int color) {
        ...
        while (i < len) {
            int i2 = i + 1;
            int dr = r - (this.colorTab[i] & UnsignedBytes.MAX_VALUE);
            int i3 = i2 + 1;
            int dg = g - (this.colorTab[i2] & UnsignedBytes.MAX_VALUE);
            int db = b - (this.colorTab[i3] & UnsignedBytes.MAX_VALUE);

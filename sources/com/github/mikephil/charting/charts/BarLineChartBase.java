...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends IBarLineScatterCandleBubbleDataSet<? extends Entry>>> extends Chart<T> implements BarLineScatterCandleBubbleDataProvider {
    ...
    public void moveViewToAnimated(float xValue, float yValue, AxisDependency axis, long duration) {
        ...
        float f = xValue;
        addViewportJob(AnimatedMoveViewJob.getInstance(this.mViewPortHandler, f, yValue + ((getAxisRange(axis) / this.mViewPortHandler.getScaleY()) / 2.0f), getTransformer(axis), this, (float) bounds.x, (float) bounds.y, duration));
    ...
    public void centerViewTo(float xValue, float yValue, AxisDependency axis) {
        ...
        addViewportJob(MoveViewJob.getInstance(this.mViewPortHandler, xValue - (xInView / 2.0f), ((getAxisRange(axis) / this.mViewPortHandler.getScaleY()) / 2.0f) + yValue, getTransformer(axis), this));
    }
    ...
    public void centerViewToAnimated(float xValue, float yValue, AxisDependency axis, long duration) {
        MPPointD bounds = getValuesByTouchPoint(this.mViewPortHandler.contentLeft(), this.mViewPortHandler.contentTop(), axis);
        float xInView = getXAxis().mAxisRange / this.mViewPortHandler.getScaleX();
        addViewportJob(AnimatedMoveViewJob.getInstance(this.mViewPortHandler, xValue - (xInView / 2.0f), ((getAxisRange(axis) / this.mViewPortHandler.getScaleY()) / 2.0f) + yValue, getTransformer(axis), this, (float) bounds.x, (float) bounds.y, duration));
        MPPointD.recycleInstance(bounds);
    }
    ...
    /* access modifiers changed from: protected */
    public float getAxisRange(AxisDependency axis) {

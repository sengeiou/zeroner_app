...
public final class FlowableWindowTimed<T> extends AbstractFlowableWithUpstream<T, Flowable<T>> {
                ...
                if (d && (empty || sw)) {
                    ...
                    if (!work.open) {
                        ...
                        if (r != 0) {
                            ...
                            Completion completion = new Completion(w3);
                            worker2.schedule(completion, this.timespan, this.unit);
                        } else {
    ...
    public void subscribeActual(Subscriber<? super Flowable<T>> s) {
        ...
        if (this.timespan != this.timeskip) {
            this.source.subscribe((FlowableSubscriber<? super T>) new WindowSkipSubscriber<Object>(actual, this.timespan, this.timeskip, this.unit, this.scheduler.createWorker(), this.bufferSize));
        } else if (this.maxSize == LongCompanionObject.MAX_VALUE) {
            this.source.subscribe((FlowableSubscriber<? super T>) new WindowExactUnboundedSubscriber<Object>(actual, this.timespan, this.unit, this.scheduler, this.bufferSize));
        } else {
            this.source.subscribe((FlowableSubscriber<? super T>) new WindowExactBoundedSubscriber<Object>(actual, this.timespan, this.unit, this.scheduler, this.bufferSize, this.maxSize, this.restartTimerOnMaxSize));
        }
    }
}

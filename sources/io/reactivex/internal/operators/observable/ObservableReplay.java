...
public final class ObservableReplay<T> extends ConnectableObservable<T> implements HasUpstreamObservableSource<T>, Disposable {
    ...
    public static <U, R> Observable<R> multicastSelector(Callable<? extends ConnectableObservable<U>> connectableFactory, Function<? super Observable<U>, ? extends ObservableSource<R>> selector) {
        return RxJavaPlugins.onAssembly((Observable<T>) new MulticastReplay<T>(connectableFactory, selector));
    }

    public static <T> ConnectableObservable<T> observeOn(ConnectableObservable<T> co, Scheduler scheduler) {
        return RxJavaPlugins.onAssembly((ConnectableObservable<T>) new Replay<T>(co, co.observeOn(scheduler)));
    }
    ...
    public static <T> ConnectableObservable<T> create(ObservableSource<T> source2, int bufferSize) {
        ...
        return create(source2, (BufferSupplier<T>) new ReplayBufferSupplier<T>(bufferSize));
    }
    ...
    public static <T> ConnectableObservable<T> create(ObservableSource<T> source2, long maxAge, TimeUnit unit, Scheduler scheduler, int bufferSize) {
        return create(source2, (BufferSupplier<T>) new ScheduledReplaySupplier<T>(bufferSize, maxAge, unit, scheduler));
    }
    ...
    static <T> ConnectableObservable<T> create(ObservableSource<T> source2, BufferSupplier<T> bufferFactory2) {
        ...
        return RxJavaPlugins.onAssembly((ConnectableObservable<T>) new ObservableReplay<T>(new ReplaySource<>(curr, bufferFactory2), source2, curr, bufferFactory2));
    }
